...
try {
  ...
  await page.locator('.forecast-progress [data-stage="eot20"][data-status="running"]').waitFor();
  await page.getByText("风速 / 阵风").waitFor();
  await page.getByText("EOT20 潮汐正在后台计算").waitFor();
  await page.getByText("临时环境评分", { exact: true }).waitFor();
  await page.getByText("当前是未计潮汐的临时环境评分：76", { exact: true }).waitFor();
  await page.getByText("临时可用时段（待潮汐确认）", { exact: true }).waitFor();
  ...
  await page.locator('.forecast-progress [data-stage="scoring"][data-status="completed"]').waitFor();
  await page.locator(".tide-score-flow").waitFor();
  ...
  await page.getByRole("button", { name: "我的钓点" }).click();
  await page.locator(".remove-favorite").click();
  await page.getByRole("button", { name: "确认取消收藏" }).click();
  await page.locator(".empty-saved").waitFor();
  await page.getByRole("button", { name: "钓鱼日志" }).click();
  await page.locator(".secondary-view article").waitFor();
  await context.close();
