...
同一坐标和预测时段的天气、Marine、降雨及 EOT20 物理结果会缓存复用。切换“钓点类型”或“钓法”时走快速适用性重评，不重新冷启动整套 Provider；只有从淡水等不适用类型切换到需要新增海况数据的类型时才补取此前未请求的数据。钓法不会改变天气或潮汐原始值，只按钓点类型、风、阵风、可用浪况与昼夜条件产生单独展示、限制在 -6 至 +4 分的适配修正；缺少相关浪况时还会限制适配分上限，避免把经验规则或缺失数据伪装成确定性鱼口结论。
