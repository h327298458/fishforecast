...
export function InvitationManager() {
  const [items, setItems] = useState<Invitation[]>([]), [maxUses, setMaxUses] = useState(1), [expiresAtUtc, setExpiresAtUtc] = useState(""), [issuedCode, setIssuedCode] = useState(""), [error, setError] = useState(""), [busy, setBusy] = useState(false);
  ...
  return <section className="invitation-manager"><h2><KeyRound /> 邀请码管理</h2><p>生成后只显示一次明文邀请码。请通过可信渠道发送给受邀用户。</p>
    ...
    <div className="invite-list"><div className="invite-list-head"><b>最近邀请码</b><button title="刷新" onClick={() => void load()}><RefreshCw /></button></div>{items.length ? items.map((item) => { const expired = item.expiresAtUtc && new Date(item.expiresAtUtc).getTime() <= Date.now(); const inactive = Boolean(item.revokedAtUtc || expired || item.uses >= item.maxUses); return <article key={item.id}><span><b>{item.uses}/{item.maxUses} 次</b><small>创建者 {item.createdByUsername} · {new Date(item.createdAtUtc).toLocaleString()}</small><small>{item.revokedAtUtc ? "已撤销" : expired ? "已过期" : item.uses >= item.maxUses ? "已用完" : item.expiresAtUtc ? `到期 ${new Date(item.expiresAtUtc).toLocaleString()}` : "不过期"}</small></span>{inactive ? null : <button title="撤销" disabled={busy} onClick={() => void revoke(item.id)}><Trash2 />撤销</button>}</article>; }) : <p>暂无邀请码。</p>}</div>
  </section>;
}
